...
async def add_movie_actors():
    async with async_session() as session:
        stmt = movie_actors.insert().values(movie_id=6, actor_id=4)
...
async def add_series_actors():
    async with async_session() as session:
        trm = series_actors.insert().values(series_id=2, actors_id=3)
        await session.execute(trm)
        await session.commit()
    
async def add_series_genre():
    async with async_session() as session:
        trm = series_genre.insert().values(series_id=1, genre_id=3)
        await session.execute(trm)
        await session.commit()
