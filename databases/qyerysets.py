...
async def add_movie_actors():
    async with async_session() as session:
        stmt = movie_actors.insert().values(movie_id=2, actor_id=1)
        await session.execute(stmt)
        await session.commit()

async def add_movie_directors():
    async with async_session() as session:
        trm = movie_directors.insert().values(movies_id=2, directors_id=2)
        await session.execute(trm)
        await session.commit()

async def add_movie_genre():
    async with async_session() as session:
        trm = movie_genre.insert().values(movies_id=2, genre_id=3)
        await session.execute(trm)
        await session.commit()

async def add_series_directors():
    async with async_session() as session:
        trm = series_directors.insert().values(series_id=2, directors_id=2)
        await session.execute(trm)
        await session.commit()

async def add_series_actors():
    async with async_session() as session:
        trm = series_actors.insert().values(series_id=2, actors_id=2)
        await session.execute(trm)
        await session.commit()
    
async def add_series_genre():
    async with async_session() as session:
        trm = series_genre.insert().values(series_id=2, genre_id=3)
        await session.execute(trm)
        await session.commit()
